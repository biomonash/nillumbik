...
import { getAllSpecies, getObservationBlocks, getObservationStats, type GetObservationBlocksResponse, type GetObservationStatsResponse, type Species, type ChartInput } from "../../../apis/mapCharts.api";
...
const MapCharts: React.FC = () => {
  ...
  function extractTaxaOptions(data: GetObservationStatsResponse): ChartInput[] {
    const taxa = Object.keys(data.countByTaxa).sort().map((t) => ({ value: t, label: capitalize(t) }));
    return [{ value: "all", label: "All Taxa" }, ...taxa];
  }

  function extractSpeciesOptions(allSpecies: Species[], selectedTaxa: string | null): ChartInput[] {
    const filtered = selectedTaxa
      ? allSpecies.filter((s) => s.taxa.toLowerCase() === selectedTaxa.toLowerCase())
      : allSpecies;
    const unique = [...new Map(filtered.map((s) => [s.commonName, s])).values()];
    return [{ value: "", label: "All Species" }, ...unique.map((s) => ({ value: s.commonName, label: s.commonName }))];
  }
 ...
 // Loads initial data
  useEffect(() => {
    getObservationBlocks({ from: DEFAULT_FROM }).then(data => setZoneOptions(extractSortedZones(data)));
    getObservationStats({ from: DEFAULT_FROM }).then(data => setTaxaOptions(extractTaxaOptions(data)));
    getAllSpecies().then(setAllSpecies);
  }, []);

  useEffect(() => {
    setSpeciesOptions(extractSpeciesOptions(allSpecies, selectedTaxa !== "all" ? selectedTaxa : null));
  ...
  useEffect(() => {
    Promise.all([getObservationBlocks(params), getObservationStats(params)])
      .then(([blocksData, statsData]) => {
        const total = blocksData.blocks.reduce((sum, b) => sum + b.observationCount, 0);
  ...
  const content = (
    <>
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-black text-lg font-semibold tracking-tight">Zone Filter 🔎</h1>
        <button
          onClick={handleReset}
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col">
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-600 mb-1">Zone</span>
          <Select options={zoneOptions} value={selectedZone} onChange={(val) => setSelectedZone(val)} placeholder="Select Zone" className="w-full" />
        </div>
        <div className="flex flex-col">
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-600 mb-1">Taxa <span className="normal-case font-normal text-gray-400">(optional)</span></span>
          <Select options={taxaOptions} value={selectedTaxa} onChange={setSelectedTaxa} placeholder="Select Taxa" className="w-full" />
        </div>
        <div className="flex flex-col">
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-600 mb-1">Species <span className="normal-case font-normal text-gray-400">(optional)</span></span>
          <Select options={speciesOptions} value={selectedSpecies} onChange={setSelectedSpecies} placeholder="Select Species" disabled={selectedTaxa === "all"} className="w-full" />
        </div>
      </div>
      ...
      {selectedSpecies && selectedSpecies !== "" && (
        <div className="flex items-center gap-2">
          ...
          <span className={`px-3 py-1 rounded-full text-white text-xs font-semibold ${nativeCount > 0 ? "bg-green-500" : "bg-red-500"}`}>
      ...
      <div className="bg-white/50 rounded-xl p-3 flex flex-col gap-1">
        ...
        <div className="flex items-baseline gap-1">
          <span className="text-2xl font-bold text-black">{observationCount.toLocaleString()}</span>
      ...
      {(!selectedSpecies || selectedSpecies === "") && (
        <div>
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-2 block">Distribution</span>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-green-50 rounded-xl p-3 text-center">
              <div className="text-xl font-bold text-green-700">{nativeCount}</div>
              <div className="text-xs text-green-600 font-medium">Native</div>
            </div>
            <div className="bg-red-50 rounded-xl p-3 text-center">
              <div className="text-xl font-bold text-red-500">{nonNativeCount}</div>
      ...
      <div className="flex flex-col gap-4">
        <Card className="text-black border-0 shadow-sm">
          <CardHeader className="pb-1 pt-3 px-3">
            ...
            <CardDescription className="text-xs">Species distribution</CardDescription>
          </CardHeader>
          <CardContent className="px-2 pb-3">
            {(!selectedSpecies || selectedSpecies === "") ? (
              <NativeBarChart nativeCount={nativeCount} nonNativeCount={nonNativeCount} />
            ) : (
              <p className="text-xs text-gray-400 text-center py-4">Clear species to see distribution</p>
            )}
          </CardContent>
        </Card>
        <Card className="text-black border-0 shadow-sm">
          ...
          <CardContent className="px-2 pb-3">
            <SpeciesLineChart filters={params} selectedSpecies={selectedSpecies} />
          </CardContent>
        </Card>
      </div>
    </>
  ...
  return (
    <>
      ...
      <div className={`md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[var(--muted-foreground2)] rounded-t-2xl shadow-xl transition-transform duration-300 ease-in-out ${drawerOpen ? "translate-y-0" : "translate-y-[calc(100%-56px)]"}`}>
        {/* Handle bar */}
        <div className="flex justify-between items-center px-4 h-14 cursor-pointer" onClick={() => setDrawerOpen(!drawerOpen)}>
          <div className="flex items-center gap-2">
            <span className="font-semibold text-black text-sm">Zone Filter 🔎</span>
            <span className="text-gray-400 text-xs">·</span>
            <span className="text-gray-500 text-xs">{observationCount.toLocaleString()} detections</span>
          </div>
